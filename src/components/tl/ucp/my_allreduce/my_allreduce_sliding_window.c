...
#include "my_allreduce_sliding_window.h"
...
static inline void
ucc_tl_ucp_my_allreduce_sliding_window_reset_buf(ucc_tl_ucp_my_allreduce_sw_buf_t *buf)
{
    buf->state   = FREE;
    buf->count   = 0;
    buf->bytes   = 0;
...
static inline void ucc_tl_ucp_my_allreduce_sliding_window_reset_pipeline(
    ...
    ucc_rank_t put_window_size)
{
    ...
    ucc_tl_ucp_my_allreduce_sliding_window_reset_buf(&pipe->accbuf);
    for (i = 0; i < pipe->num_buffers; i++) {
        ucc_tl_ucp_my_allreduce_sliding_window_reset_buf(&pipe->getbuf[i]);
    }
...
ucc_status_t
ucc_tl_ucp_my_allreduce_sliding_window_start(ucc_coll_task_t *coll_task)
{
    ...
    ucc_tl_ucp_my_allreduce_sliding_window_reset_pipeline(
...
ucc_status_t
ucc_tl_ucp_my_allreduce_sliding_window_finalize(ucc_coll_task_t *coll_task)
{
...
ucc_status_t
ucc_tl_ucp_my_allreduce_sliding_window_rdma_task_post(
...
static inline void ucc_tl_ucp_my_allreduce_sliding_window_free_rkeys(
    ucc_coll_task_t *coll_task)
{
...
static ucc_status_t
ucc_tl_ucp_my_allreduce_sliding_window_rdma_task_finalize(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ucc_status_t       st   = UCC_OK;

    ucc_tl_ucp_my_allreduce_sliding_window_free_rkeys(coll_task);
    ucc_tl_ucp_my_allreduce_sliding_window_free_task(coll_task);
    ucc_tl_ucp_my_allreduce_sliding_window_free_pipe(coll_task);
...
static inline void ucc_tl_ucp_my_allreduce_sliding_window_reduction(
    ucc_coll_task_t *coll_task, ucc_tl_ucp_my_allreduce_sw_buf_t *accbuf,
    ucc_tl_ucp_my_allreduce_sw_buf_t *getbuf)
{
    ucc_status_t       status = UCC_OK;
    ucc_tl_ucp_task_t *task   = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
...
static inline void
ucc_tl_ucp_my_allreduce_sliding_window_test_reduction(ucc_tl_ucp_task_t *task)
{
    ucc_status_t status;
...
ucc_tl_ucp_my_allreduce_sliding_window_req_test(ucs_status_ptr_t   request,
...
static inline void ucc_tl_ucp_my_allreduce_sliding_window_key_exchange_progress(
    ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t *task      = ucc_derived_of(coll_task, ucc_tl_ucp_task_t);
    ...
    UCC_CHECK_GOTO(
        ucc_tl_ucp_my_allreduce_sliding_window_allgather_info_finalize(task),
        err, status);
...
static inline void ucc_tl_ucp_my_allreduce_sliding_window_mark_redbuf_free(
    ucc_tl_ucp_allreduce_sw_pipeline_t *pipe,
    ucc_tl_ucp_my_allreduce_sw_buf_t      *accbuf,
    ucc_tl_ucp_my_allreduce_sw_buf_t      *redbuf,
...
void ucc_tl_ucp_my_allreduce_sliding_window_rdma_progress(ucc_coll_task_t *coll_task)
{
    ucc_tl_ucp_task_t                  *task            =
    ...
    ucc_tl_ucp_my_allreduce_sw_buf_t      *accbuf          = &pipe->accbuf;
    ...
    if (allgather_task != NULL) {
        ucc_tl_ucp_my_allreduce_sliding_window_key_exchange_progress(coll_task);
        return;
    }
    ...
    if (*reduce_task != NULL) {
        ...
        // completed.
        ucc_tl_ucp_my_allreduce_sliding_window_test_reduction(task);
        ...
        ucc_tl_ucp_my_allreduce_sliding_window_mark_redbuf_free(
    ...
    if (pipe->count_serviced < pipe->my_count) {
        ...
        if (accbuf->state == RECVING) {
            ...
            status = ucc_tl_ucp_my_allreduce_sliding_window_req_test(request, task);
            if (status == UCC_OK) {
                if (request)
        ...
        if (accbuf->state == REDUCING && redbuf->state == RECVING) {
            ...
            if (status == UCC_OK) {
                ...
                ucc_tl_ucp_my_allreduce_sliding_window_reduction(coll_task, accbuf,
                                                              redbuf);
        ...
        if ((pipe->count_serviced < pipe->count_reduced) &&
            ...
            for (i = pipe->done_put; i < pipe->posted_put; i++) {
                ...
                // too
                if (ucc_tl_ucp_my_allreduce_sliding_window_req_test(
                        request, task) != UCC_OK)
                    break;
            ...
            if (pipe->done_put == host_team_size) {
                ...
                ucc_tl_ucp_my_allreduce_sliding_window_reset_buf(accbuf);
                pipe->done_get = 0;
...
ucc_status_t
ucc_tl_ucp_my_allreduce_sliding_window_init(ucc_base_coll_args_t *coll_args,
                                         ...
                                         ucc_coll_task_t     **task_h)
{
    ...
    *task_h                  = &schedule->super;
    schedule->super.post     = ucc_tl_ucp_my_allreduce_sliding_window_start;
    schedule->super.progress = NULL;
    schedule->super.finalize = ucc_tl_ucp_my_allreduce_sliding_window_finalize;
    ...
    if (ucc_tl_ucp_my_allreduce_sliding_window_alloc_pipe(team, rdma_task) != UCC_OK) {
        tl_error(UCC_TL_TEAM_LIB(tl_team), "failed to alloc pipe: %s",
                                 ucc_status_string(status));
        goto free_rdma_task;
    }

    status = ucc_tl_ucp_my_allreduce_sliding_window_task_init(coll_args, team,
                                                           rdma_task);
    if (status != UCC_OK) {
    ...
    rdma_task->super.post     = ucc_tl_ucp_my_allreduce_sliding_window_rdma_task_post;
    rdma_task->super.progress = ucc_tl_ucp_my_allreduce_sliding_window_rdma_progress;
    rdma_task->super.finalize = ucc_tl_ucp_my_allreduce_sliding_window_rdma_task_finalize;
...
free_rdma_pipe:
    ucc_tl_ucp_my_allreduce_sliding_window_free_pipe(&rdma_task->super);
free_rdma_task:
    ucc_tl_ucp_my_allreduce_sliding_window_free_task(&rdma_task->super);
out:
    ucc_tl_ucp_put_schedule(schedule);
    return status;
}
